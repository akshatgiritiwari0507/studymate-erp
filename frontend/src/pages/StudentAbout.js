...
export default function StudentAbout() {
  ...
  useEffect(()=>{
    (async()=>{
      try { const r = await api.get('/student/me/profile'); setForm({ fullName: r.data.fullName||'', mobile: r.data.mobile||'', address: r.data.address||'' }); } catch {}
    })();
  },[]);

  const save = async (e) => {
    e.preventDefault(); setStatus('');
    await api.post('/student/me/profile', form);
